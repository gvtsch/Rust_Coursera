...
struct Car {
    ...
    motor: Transmission,
    roof: bool,
    age: (Age, u32) //tuple with Age enum, u32, // todo!("Move `mileage: u32` field into `age` field - a tuple with two fields: an `Age` enum, u32");
}
...
// Add enum for Car age type new or used 
#[derive(PartialEq, Debug)]
enum Age { New, Used }
